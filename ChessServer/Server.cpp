...
Server::Server(int PORT, bool BroadcastPublically)  // Port = port to broadcast on. BroadcastPublically = false if server is not open to the public (people outside of your router), true = server is open to everyone (assumes that the port is properly forwarded on router settings)
{
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        log("Error: Failed to open the file accs.txt.");
        exit(0);
    }
    QTextStream in(&file);
    if (!in.atEnd())
    ...
    if (accsFilePath.length() == 0) {
        ...
        log("Error: You don't have account file path.");
        exit(0);
    }
    ...
    if (file.exists()) {
        if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            log("Error: Failed to open the file " + accsFilePath.toStdString());
            exit(0);
        }
        bool isID = true;
        Account tmpAcc;
        ...
        if (!file.open(QIODevice::WriteOnly)) {
            ...
            log("Error: Failed to create the file " + accsFilePath.toStdString());
